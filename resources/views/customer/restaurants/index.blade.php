    ...
    <section class="bg-gradient-to-r from-green-50 to-blue-50 py-20 px-4 md:px-10 rounded-3xl shadow-sm">
        <div class="max-w-4xl mx-auto text-center">
            <h1 class="text-4xl md:text-5xl font-extrabold text-gray-800 mb-6 leading-tight">
                Discover Healthy <span
                    class="bg-clip-text text-transparent bg-gradient-to-r from-green-600 to-blue-500">Dining Options</span>
            </h1>
        ...
        <div class="max-w-7xl mx-auto">
            <div class="flex justify-between items-center mb-10">
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    <button class="filter-btn active px-4 py-2 bg-green-600 text-white rounded-full text-sm font-medium"
                    ...
                    @php
                        $uniqueCategories = $restaurants->pluck('category.name')->unique()->filter();
                    @endphp

                    @foreach ($uniqueCategories as $categoryName)
                        <button
                            class="filter-btn px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-full text-sm font-medium transition"
                            data-filter="{{ strtolower($categoryName) }}">
                            {{ $categoryName }}
                        </button>
                    @endforeach
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($restaurants as $restaurant)
                    <div class="restaurant-card bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
                        data-category="{{ strtolower($restaurant->category->name) }}">
                        <div class="relative overflow-hidden h-56">
                            <img src="{{ asset('storage/restaurants/' . $restaurant->restaurant_image) }}"
                                alt="{{ $restaurant->name }}"
                            ...
                            <div class="absolute top-4 right-4">
                                <button
                                    class="heart-btn p-2 bg-white bg-opacity-80 rounded-full shadow-sm hover:bg-red-100 transition">
